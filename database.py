...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS roster_players (
            ...
            UNIQUE(roster_id, player_name)
        );

        CREATE INDEX IF NOT EXISTS idx_roster_id ON roster_players(roster_id);
    """)
...
def create_roster(name: str) -> int:
    """Create a new roster. Returns the roster id."""
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO rosters (name, created_at) VALUES (?, ?)",
...
def get_all_rosters():
    """Return all rosters with player counts."""
    conn = get_db()
    rows = conn.execute("""
        SELECT r.*,
...
def get_roster(roster_id: int):
    """Get a roster with its players and their aggregated stats."""
    conn = get_db()
    roster = conn.execute(
    ...
    # Get all player names in this roster
    rp_rows = conn.execute("""
        SELECT player_name FROM roster_players
        WHERE roster_id = ?
    ...
    for rp in rp_rows:
        ...
        if stats and stats["matches"]:
            ...
            # Player exists in roster but has no match data yet
            players.append({
                "player_name": name,
                "matches": 0,
                "avg_overall": 0,
...
def add_player_to_roster(roster_id: int, player_name: str) -> bool:
    """Add a player to a roster. Returns True if added, False if already exists."""
    conn = get_db()
    try:
        conn.execute(
            "INSERT INTO roster_players (roster_id, player_name, added_at) VALUES (?, ?, ?)",
            (roster_id, player_name.strip(), datetime.now().isoformat()),
        )
        conn.commit()
        conn.close()
        return True
...
def remove_player_from_roster(roster_id: int, player_name: str) -> bool:
    """Remove a player from a roster. Returns True if removed."""
    conn = get_db()
    cur = conn.execute(
        "DELETE FROM roster_players WHERE roster_id = ? AND LOWER(player_name) = LOWER(?)",
...
def rename_roster(roster_id: int, new_name: str) -> bool:
    """Rename a roster."""
    conn = get_db()
    cur = conn.execute(
        "UPDATE rosters SET name = ? WHERE id = ?",
        (new_name.strip(), roster_id),
    )
    conn.commit()
...
def delete_roster(roster_id: int) -> bool:
    """Delete a roster and all its player associations."""
    conn = get_db()
    conn.execute("DELETE FROM roster_players WHERE roster_id = ?", (roster_id,))
