...
def create_roster():
    ...
    if name:
        roster_id = db.create_roster(name)
        return redirect(url_for("roster_detail", roster_id=roster_id))
    return redirect(url_for("rosters_page"))


@app.route("/roster/<int:roster_id>")
def roster_detail(roster_id):
    """View a roster with all its players."""
    roster, players = db.get_roster(roster_id)
    if not roster:
        return "Roster not found", 404
    all_players = db.get_all_player_names()
    return render_template("roster_detail.html", roster=roster, players=players,
                           all_players=all_players)


@app.route("/roster/<int:roster_id>/add", methods=["POST"])
def roster_add_player(roster_id):
    """Add a player to a roster."""
    player_name = request.form.get("player_name", "").strip()
    if player_name:
        db.add_player_to_roster(roster_id, player_name)
    return redirect(url_for("roster_detail", roster_id=roster_id))


@app.route("/roster/<int:roster_id>/remove", methods=["POST"])
def roster_remove_player(roster_id):
    """Remove a player from a roster."""
    player_name = request.form.get("player_name", "").strip()
    if player_name:
        db.remove_player_from_roster(roster_id, player_name)
    return redirect(url_for("roster_detail", roster_id=roster_id))


@app.route("/roster/<int:roster_id>/rename", methods=["POST"])
def roster_rename(roster_id):
    """Rename a roster."""
    new_name = request.form.get("name", "").strip()
    if new_name:
        db.rename_roster(roster_id, new_name)
    return redirect(url_for("roster_detail", roster_id=roster_id))
